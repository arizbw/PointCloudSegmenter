...
def room2blocks(data, label, num_point, block_size=1.0, stride=1.0,
    ...
    """
    room_xyz_min = np.amin(data[:, 0:3], axis=0)
    data[:, 0:3] -= room_xyz_min
    ...
    for idx in range(len(xbeg_list)): 
       ...
       globalSample.append(np.expand_dims(block_data_sampled, 0))
...
def processFromFile(path):
    data, sample = room2blocks_wrapper_normalized(path, 4096)
    return data
